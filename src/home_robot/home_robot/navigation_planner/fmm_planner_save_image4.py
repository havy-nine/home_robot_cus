    ...
    def set_multi_goal(
        ...
        if self.visualize or self.print_images and timestep != 0:
            ...
            if self.print_images and timestep is not None:
                cv2.imwrite(
                    os.path.join(self.vis_dir, f"traversible_{timestep}.png"),
                    (np.flipud(traversible) * 255).astype(int),
                )
                cv2.imwrite(
                    os.path.join(self.vis_dir, f"goal_map_{timestep}.png"),
                    (np.flipud(goal_map) * 255).astype(int),
                )
                cv2.imwrite(
                    os.path.join(self.vis_dir, f"fmm_dist_{timestep}.png"),
                    (np.flipud(self.fmm_dist / self.fmm_dist.max()) * 255).astype(int),
                )
